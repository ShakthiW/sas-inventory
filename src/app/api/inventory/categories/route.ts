...
const categorySchema: z.ZodType<CategoryCreatePayload> = z.object({
  name: z.string().min(1),
...
export async function GET(request: NextRequest) {
  ...
  const sort =
    (searchParams.get("sort") as CategorySortField | null) || "createdAt"; // createdAt | name
  const dir =
  ...
  if (q) {
    ...
    andClauses.push({ $or: [{ name: regex }, { slug: regex }] });
  }
